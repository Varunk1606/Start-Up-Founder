...
const Dashboard = () => {
    ...
    const getData = async () => {
        ...
        if (!u) {
            ...
            // If user missing in DB, sign out / redirect to login
            try {
                signOut()
            } catch (err) {
                router.push('/login')
    ...
    const handleSubmit = async (e) => {
        ...
        if (result?.success) {
            // Use the new username (if provided) to refresh the user data
            let newUsername = form.username || session.user.name
            let updatedUser = await fetchuser(newUsername)
            if (updatedUser) {
                setform(updatedUser)
                // If username changed, update the next-auth session so other pages use the new name
                try {
                    if (newUsername !== session.user.name) {
                        await update({ ...session, user: { ...session.user, name: newUsername } })
                    }
                } catch (err) {
                    // silently ignore session update failure; UI already updated
                    console.error('Failed to update session after username change', err)
                }
            }
            ...
            toast(result?.error || 'Failed to update profile', {
                position: "top-right",
                autoClose: 5000,
                hideProgressBar: false,
                closeOnClick: true,
