...
const PaymentPage = ({ username }) => {
    ...
    useEffect(() => {
        // Show toast only when paymentdone=true is present in the query string.
        // Then replace the URL (remove query) after a short delay so the toast is visible.
        const paid = searchParams.get("paymentdone")
        if (paid === "true") {
            ...
            // Give the toast a moment to render before cleaning the URL
            setTimeout(() => {
    ...
    const pay = async (amount) => {
        ...
        var options = {
            ...
            "currency": "INR",
            "name": "Start-Up Founder", //your business name
    ...
    return (
        <>
            ...
            <div className='cover w-full bg-red-50 relative'>
                <img className='object-cover w-full h-48 md:h-[400px] shadow-blue-700 shadow-sm' src={currentUser.coverpic} alt="" />
                        ...
                        <ul className='mx-5 text-lg'>
                            ...
                            {payments.map((p, i) => {
                                return <li key={i} className='my-4 flex gap-2 items-center'>
                                    <img width={33} src="icons/avatar.gif" alt="user avatar" />
                                    <span className='text-lg'>
                                        {p.name} donated <span className='font-bold text-md'>₹{p.amount}</span> with a message &quot;{p.message}&quot;
                                    </span>
                                </li>
                            })}
